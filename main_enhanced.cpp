...
// Frame rate limiter to prevent overwhelming the system - NVIDIA driver stability fix
static void limitFrameRate() {
    ...
    // More aggressive frame rate limiting for NVIDIA driver stability
    if (elapsed < targetFrameTime) {
        auto sleepTime = targetFrameTime - elapsed;
        std::this_thread::sleep_for(sleepTime);
    }
    
    // Additional stability: Force occasional driver sync
    framesSinceLastCheck++;
    if (framesSinceLastCheck >= 60) { // Every second at 60fps
        glFinish(); // Force GPU sync to prevent driver queue overflow
...
static void updatePerformance() {
    ...
    if (perf.frameCount > 1000) { // CRITICAL FIX: Reset much more frequently (1000 vs 10000)
        perf.frameCount = 1;
...
int main(int argc, char* argv[]) {
    ...
    // NVIDIA Driver Stability: Additional initialization checks
    if (!glGetString(GL_VERSION) || !glGetString(GL_RENDERER)) {
        std::cerr << "ERROR: OpenGL context not properly initialized" << std::endl;
        return 1;
    }
...
static void bindUniforms(GLuint prog) {
    ...
    if (uFrameCount >= 0) glUniform1i(uFrameCount, perf.frameCount % 512); // CRITICAL: Modulate to prevent precision issues
...
static void Handle_Display() {
    // NVIDIA Driver Stability: Extra safeguards
    static int consecutiveErrors = 0;
    static int framesSinceLastError = 0;
    ...
    if (!checkGLError("Display function start")) {
        consecutiveErrors++;
        if (consecutiveErrors > 5) {
            std::cerr << "Too many consecutive OpenGL errors, attempting recovery..." << std::endl;
            // Force reset OpenGL state
            glUseProgram(0);
        ...
        if (framesSinceLastError > 60) { // Reset error counter after 1 second of no errors
            consecutiveErrors = 0;
        }
    }
    ...
    // CRITICAL: Additional error checking after buffer swap for NVIDIA stability
    if (!checkGLError("After glutSwapBuffers")) {
        // If buffer swap causes errors, force a clean state
        glFinish(); // Force completion of all OpenGL commands
        glBindTexture(GL_TEXTURE_2D, 0);
        glBindVertexArray(0);
        glUseProgram(0);
